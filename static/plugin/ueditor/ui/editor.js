    ...
    initEditorUI: function () {
      ...
      editor.addListener('ready', function () {
        ...
        if (editor.options.elementPathEnabled) {
          editor.ui.getDom(
            'elementpath').innerHTML = '<div class="edui-editor-breadcrumb">' +
      ...
      function setCount (editor, ui) {
        ...
        if (count > max) {
          ...
          countDom.innerHTML = msg.replace('{#leave}', max - count).
            replace('{#count}', count)
        }
      }
      ...
      if (editor.options.imagePopup) {
        editor.addListener('mouseover', function (t, evt) {
          ...
          //是否为按钮
          var ifInput = el.className.indexOf('extend') > -1 ||
            el.parentElement.className.indexOf('extend') > -1
          var ifElParent = el.parentElement.getAttribute('name') ==
            'editable-input'
          var onlydelete = el.className.indexOf('onlydelete') > -1 ||
            el.parentElement.className.indexOf('onlydelete') > -1
          ...
          if (isinput) {
            ...
            readOnly = (el.childNodes[0].tagName == 'INPUT' ||
              el.childNodes[0].tagName == 'TEXTAREA') &&
            el.childNodes[0].getAttribute('readonly') ? true : false
          }
          var isSelect = false
          ...
          if (isNowSelect) {
            ...
            if (hascompiler) {
              ...
              isCascadequery = el.childNodes[0].getAttribute('selectquery')
                ? true
                : false
            }
          }
          ...
          if (ifEl || ifElParent) {
            if (readOnly) {
              if (isflag) {
                ...
                var html = popup.formatHtml(
                  '<nobr>' + editor.getLang('property') +
                  ': <span onclick="$$._onInputDelButtonClick()" class="edui-clickable">' +
                  editor.getLang('delete') + '</span>&nbsp;&nbsp;' +
            ...
            if (ifInput) {
              html = popup.formatHtml(
                ...
                editor.getLang('customdialog.self') + '</span>' +
                '</nobr>')
            }
            if (onlydelete) {
              html = popup.formatHtml(
        ...
        editor.addListener('selectionchange', function (t, causeByUi) {
          ...
          if (editor.ui._dialogs.linkDialog) {
            ...
            var url
            if (link && (url = (link.getAttribute('data_ue_src') ||
    ...
    setFullScreen: function (fullscreen) {
      ...
      if (this._fullscreen != fullscreen) {
        ...
        if (fullscreen) {
          ...
          this._bakScrollTop = Math.max(document.documentElement.scrollTop,
    ...
    _updateFullScreen: function () {
      if (this._fullscreen) {
        ...
        this.getDom().style.cssText = 'border:0;position:absolute;left:0;top:' +
          (this.editor.options.topOffset || 0) + 'px;width:' + vpRect.width +
          'px;height:' + vpRect.height + 'px;z-index:' +
    ...
    _updateElementPath: function () {
      ...
      if (this.elementPathEnabled &&
        ...
        for (var i = 0, ci; ci = list[i]; i++) {
          buff[i] = this.formatHtml('<span unselectable="on" onclick="$$.editor.execCommand(&quot;elementpath&quot;, &quot;' +
            i + '&quot;);">' + ci + '</span>')
        }
    ...
    _scale: function () {
      ...
      function down () {
        ...
        scalelayer.style.cssText = 'position:absolute;left:0;display:;top:0;background-color:#41ABFF;opacity:0.4;filter: Alpha(opacity=40);width:' +
          ...
          + editorHolder.offsetHeight + 'px;z-index:' +
        ...
        if (fullScreen) {
          ...
          if (me.editor.options.scaleEnabled) {
            ...
            me.editor.body.style.height = Math.max(
              domUtils.getXY(tmpNode).y, me.editor.iframe.offsetHeight - 20) +
              'px'
      ...
      function up () {
        if (isMouseMove) {
          ...
          editor.setHeight(scalelayer.offsetHeight - bottombar.offsetHeight -
