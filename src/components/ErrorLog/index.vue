      ...
      <el-table :data="errorLogs" border>
        <el-table-column label="报错请求信息">
          <template  slot-scope="scope">
            ...
            <br/>
            <div>
              <span class="message-title" style="padding-right: 10px;">告警: </span>
              <el-tag type="warning">{{scope.row.vm.$vnode.tag}} error in {{scope.row.info}}</el-tag>
            </div>
            <br/>
            <div>
              <span class="message-title" style="padding-right: 16px;">请求URL: </span>
              <el-tag type="success">{{scope.row.url}}</el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="具体日志">
          <template  slot-scope="scope">
            {{ scope.row.err.stack}}
          </template>
        </el-table-column>
