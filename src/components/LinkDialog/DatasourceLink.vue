    ...
    <el-dialog title="关联" :visible.sync="dialog.visible" width="85%">
      <el-tabs v-model="activeName" v-if="dialog.visible">
        <el-tab-pane label="业务" name="business">
          <data-run-link ref="linkBusiness" :btnState="btnState" :selectedData="config.selected.bussiness"></data-run-link>
        </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    saveLink() {
      // 组合数据
      const fromData = { type: 'DATASOURCE', data: this.config.selected }
      const toData = [{ type: 'BUSINESS', data: this.$refs.linkBusiness.multipleSelection },
