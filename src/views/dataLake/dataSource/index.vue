  ...
  <div class="app-container calendar-list-container">
          ...
          <el-card>
            ...
            <div class="table">
              <div>
                ...
                >
                  <el-table-column type="selection" width="55" v-if="btnState"></el-table-column>
                  <el-table-column
                    width="200"
                    align="center"
                    sortable
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form" @submit.native.prevent>
        ...
        >
          <el-input
            :type="form.type === 'Kafka'?'text':'password'"
    ...
    <!--  添加关联 -->
    <datasource-link ref="linkDialog" :config="dialog.linkConfig" @saveLink="refreshList"></datasource-link>
  </div>
</template>
...
const REQUIRED_TYPE = ['mysql', 'oracle', 'postgresql', 'db2', 'hive', 'kafka', 'dmdb']
const DATASOURCE_CONGI = {
  ...
  dmdb: 'jdbc:dm://localhost:5236/sysdba'
}
export default {
  name: 'index',
  props: ['btnState'],
  ...
  methods: {
    ...
    handleEdit (row) {
      console.log(row)
      /* if (row.bussiness.length > 0 || row.task.length > 0) {
        this.$message.warning('无法编辑,该数据源有关联');
        return;
      } */
      // resetFields重置无效
      this.initForm('form')
    ...
    handleDel (row) {
      if (row.bussiness.length > 0 || row.task.length > 0) {
        this.$message.warning('该数据源有关联信息，无法删除')
        return;
      }
      this.$confirm('确认删除该数据源?', '提示', {
        ...
        if (row.bussiness.length > 0 || row.task.length > 0) {
          this.$message.warning('无法删除,该数据源有关联');
          return;
        }
        this.listLoading = true;
    ...
    testState (row) {
      ...
      testConnect(row).then(res => {
        if (res.code !== 0) {
          this.$message.error(res.msg ? res.msg : '测试失败')
          return;
        }
